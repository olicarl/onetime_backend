...
void Evse::setup() {
  ...
  if (auto context = getOcppContext()) {
    if (context->getVersion().major == 2) {
      ...
      if (auto varService = context->getModel().getVariableService()) {
        varService->declareVariable<bool>(
            "AuthCtrlr", "LocalAuthorizeOffline", false,
            MicroOcpp::Variable::Mutability::ReadOnly, false);
      }
    }
  }
#endif
  ...
  trackEvPluggedBool = MicroOcpp::declareConfiguration(
      trackEvPluggedKey.c_str(), false, SIMULATOR_FN, false, false, false);
  snprintf(key, 30, "evsePlugged_cId_%u", connectorId);
  trackEvsePluggedKey = key;
  trackEvsePluggedBool = MicroOcpp::declareConfiguration(
      trackEvsePluggedKey.c_str(), true, SIMULATOR_FN, false, false, false);
  snprintf(key, 30, "evReady_cId_%u", connectorId);
  trackEvReadyKey = key;
  trackEvReadyBool = MicroOcpp::declareConfiguration(
      trackEvReadyKey.c_str(), false, SIMULATOR_FN, false, false, false);
  snprintf(key, 30, "evseReady_cId_%u", connectorId);
  trackEvseReadyKey = key;
  trackEvseReadyBool = MicroOcpp::declareConfiguration(
      trackEvseReadyKey.c_str(), false, SIMULATOR_FN, false, false, false);

  MicroOcpp::configuration_load(SIMULATOR_FN);

  setConnectorPluggedInput(
      [this]() -> bool {
        return trackEvPluggedBool
            ->getBool(); // return if J1772 is in State B or C
      },
      connectorId);
  ...
  setEvseReadyInput([this]() -> bool { return trackEvseReadyBool->getBool(); },
                    connectorId);

  addErrorCodeInput(
      [this]() -> const char * {
        const char *errorCode =
            nullptr; // if error is present, point to error code; any number of
                     // error code samplers can be added in this project
        return errorCode;
      },
      connectorId);

  setEnergyMeterInput([this]() -> float { return simulate_energy; },
                      connectorId);

  setPowerMeterInput([this]() -> float { return simulate_power; }, connectorId);

  addMeterValueInput([this]() { return (int32_t)getCurrent(); },
                     "Current.Import", "A", "Outlet", nullptr, connectorId);

  addMeterValueInput([this]() { return (int32_t)getVoltage(); }, "Voltage", "V",
                     nullptr, nullptr, connectorId);

  addMeterValueInput(
      [this]() { return (int32_t)(simulate_power > 1.f ? 44.f : 0.f); }, "SoC",
      nullptr, nullptr, nullptr, connectorId);

  setOnResetExecute([](bool isHard) { exit(0); });

  setSmartChargingPowerOutput(
      [this](float limit) {
        if (limit >= 0.f) {
...
void Evse::loop() {
  ...
  bool simulate_isCharging =
      ocppPermitsCharge(connectorId) && trackEvPluggedBool->getBool() &&
      trackEvsePluggedBool->getBool() && trackEvReadyBool->getBool() &&
      trackEvseReadyBool->getBool();

  simulate_isCharging &=
      limit_power >= 720.f; // minimum charging current is 6A (720W for 120V
                            // grids) according to J1772

  if (simulate_isCharging) {
    if (simulate_power >= 1.f) {
      simulate_energy += (float)(mocpp_tick_ms() - simulate_energy_track_time) *
                         simulate_power * (0.001f / 3600.f);
    }

    simulate_power = SIMULATE_POWER_CONST;
    simulate_power = std::min(simulate_power, limit_power);
    simulate_power +=
        (((mocpp_tick_ms() / 5000) * 3483947) % 20000) * 0.001f - 10.f;
...
bool Evse::presentNfcTag(const char *uid, const char *type) {
  ...
  MicroOcpp::IdToken idToken{nullptr, MicroOcpp::IdToken::Type::UNDEFINED,
                             "Simulator"};
  ...
  if (auto txService = getOcppContext()->getModel().getTransactionService()) {
    if (auto evse = txService->getEvse(connectorId)) {
      if (evse->getTransaction() &&
          evse->getTransaction()->isAuthorizationActive) {
...
int Evse::getTransactionId() {
  return getTransaction(connectorId)
             ? getTransaction(connectorId)->getTransactionId()
             : -1;
}

bool Evse::chargingPermitted() { return ocppPermitsCharge(connectorId); }

int Evse::getPower() { return (int)simulate_power; }
