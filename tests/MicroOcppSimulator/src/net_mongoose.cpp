...
// cors_headers allow the browser to make requests from any domain, allowing all
// headers and all methods
...
void server_initialize(MicroOcpp::MOcppMongooseClient *osock, const char *cert,
                       const char *key, const char *user, const char *pass) {
  ao_sock = osock;
  api_cert = cert;
  api_key = key;
  api_user = user;
...
void http_serve(struct mg_connection *c, int ev, void *ev_data) {
  if (ev == MG_EV_ACCEPT) {
    ...
    // struct mg_http_message *message_data = (struct mg_http_message *)
    // ev_data;
    struct mg_http_message *message_data =
        reinterpret_cast<struct mg_http_message *>(ev_data);
    ...
    if (!api_check_basic_auth(user, pass)) {
      mg_http_reply(c, 401, final_headers,
                    "Unauthorized. Expect Basic Auth user and / or password\n");
      return;
    }
    ...
    if (mg_match(message_data->uri, mg_str("/api/websocket"), NULL)) {
      ...
      auto webSocketPingIntervalInt = MicroOcpp::declareConfiguration<int>(
          "WebSocketPingInterval", 10, MO_WSCONN_FN);
      auto reconnectIntervalInt = MicroOcpp::declareConfiguration<int>(
          MO_CONFIG_EXT_PREFIX "ReconnectInterval", 30, MO_WSCONN_FN);
...
#define RESP_BUF_SIZE 8192
      ...
      if (status == 404) {
        status =
            mocpp_api2_call(message_data->uri.buf + strlen("/api"),
                            message_data->uri.len - strlen("/api"), method,
                            message_data->query.buf, message_data->query.len,
                            resp_buf, RESP_BUF_SIZE);
      }
      if (status == 404) {
        status =
            mocpp_api_call(message_data->uri.buf + strlen("/api"), method,
                           message_data->body.buf, resp_buf, RESP_BUF_SIZE);
      }

      mg_http_reply(c, status, final_headers, resp_buf);
    } else if (mg_match(message_data->uri, mg_str("/"),
                        NULL)) { // if no specific path is given serve dashboard
                                 // application file
