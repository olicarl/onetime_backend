...
private:
  ...
  const float SIMULATE_ENERGY_DELTA_MS =
      SIMULATE_POWER_CONST / (3600.f * 1000.f);
...
public:
  ...
  const char *getOcppStatus() { return status.c_str(); }

  unsigned int getConnectorId() { return connectorId; }

  int getEnergy() { return (int)simulate_energy; }
  ...
  int getSmartChargingMaxPower() { return limit_power; }
